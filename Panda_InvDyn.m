...
for i=1:n 
    ...
    Chain(i).C = zeros(4,4);
    Chain(i).S = zeros(6,1);
    Chain(i).Sd = zeros(6,1);
    Chain(i).S2d = zeros(6,1);
    Chain(i).S3d = zeros(6,1);
...
for i=1:N+1
    ...
    [Q(i,:),Qd(i,:),Q2d(i,:)] = InvDyn(q(i,:),qd(i,:),q2d(i,:),q3d(i,:),q4d(i,:));
end;
toc
end;
Q_spat = Q;
...
% % store results
% save('q.txt','q','-ascii')
% save('qd.txt','qd','-ascii')
% save('q2d.txt','q2d','-ascii')
% save('QQ.txt','Q','-ascii')
% save('QQd.txt','Qd','-ascii')
% save('QQ2d.txt','Q2d','-ascii')
...
xlabel('Time [s]');
ylabel('Generalized Forces Q [Nm]');
title('Inverse Dynamics')
figure(7)
plot(time,[Qd(:,1),Qd(:,2),Qd(:,3),Qd(:,4),Qd(:,5),Qd(:,6),Qd(:,7)])
xlabel('Time [s]');
ylabel('Generalized Forces Qdot [Nm/s]');
title('Inverse Dynamics First Derivative')
figure(8)
plot(time,[Q2d(:,1),Q2d(:,2),Q2d(:,3),Q2d(:,4),Q2d(:,5),Q2d(:,6),Q2d(:,7)])
xlabel('Time [s]');
ylabel('Generalized Forces Qddot [Nm/s^2]');
title('Inverse Dynamics Second Derivative')
else
...
Qd_spatappr=gradient(Q_spat') ./ gradient(time);
Qd_spatappr=Qd_spatappr';
figure(3);
plot(time,Qd_spat,'b',time,Qd_spatappr,'r--');
Qd_spatDiff=Qd_spatappr-Qd_spat;
figure(4);
plot(time,Qd_spatDiff)
...
%Q2d_spatappr=gradient(Qd_spat') ./ gradient(time);
Q2d_spatappr=gradient(Qd_spatappr') ./ gradient(time);
Q2d_spatappr=Q2d_spatappr';
figure(3);
plot(time,Q2d_spat,'b',time,Q2d_spatappr,'r--');
Q2d_spatDiff=Q2d_spatappr-Q2d_spat;
figure(4);
plot(time,Q2d_spatDiff)

%%%
Qdappr=gradient(Q) ./ gradient(time(:));
figure(1);
plot(time,Q(:,1))
figure(2);
plot(time,Qdappr(:,1),'b',time,Qd(:,1),'r--')
figure(3);
QdDiff=Qdappr-Qd;
plot(time,QdDiff(:,1));

figure(9);
plot(time,Q(:,1),'b',time,Q_body(:,1),'b--o',time,Q(:,2),'r',time,Q_body(:,2),'r--o',time,Q(:,3),'g',time,Q_body(:,3),'g--o')
figure(10);
plot(time,Qd(:,1),'b',time,Qd_body(:,1),'b--o',time,Qd(:,2),'r',time,Qd_body(:,2),'r--o',time,Qd(:,3),'g',time,Qd_body(:,3),'g--o')
figure(11);
plot(time,Q2d(:,1),'b',time,Q2d_body(:,1),'b--o',time,Q2d(:,2),'r',time,Q2d_body(:,2),'r--o',time,Q2d(:,3),'g',time,Q2d_body(:,3),'g--o')
